...
public class PlayerStatus : Status
{
    ...
    public int Strength { get => strength; set { strength = Mathf.Max(0, value); ApplyFinalStats(Attack, Defense, CurrentHealth, MoveSpeed, DropRate); } }
    public int Agility { get => agility; set { agility = Mathf.Max(0, value); ApplyFinalStats(Attack, Defense, CurrentHealth, MoveSpeed, DropRate); } }
    public int Luck { get => luck; set { luck = Mathf.Max(0, value); ApplyFinalStats(Attack, Defense, CurrentHealth, MoveSpeed, DropRate); } }
    ...
    public void ApplyFinalStats(int baseAtk, int baseDef, int baseHP, float baseMove, float baseDrop)
    {
        ...
        // ② 최종 합성
        Attack = baseAtk + addAtkFromStr + addAtkFromAgi;       // 최종 공격력
